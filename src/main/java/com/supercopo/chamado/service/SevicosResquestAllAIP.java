...
import com.supercopo.chamado.domain.Codigo;
import com.supercopo.chamado.domain.dto.CnpjDTOAPI;
import com.supercopo.chamado.repository.CodigoRepository;
...
public class SevicosResquestAllAIP {
	...
	private CodigoRepository repoCodigo;
	  ...
	  @Autowired
	    public SevicosResquestAllAIP(WebClient webClientcnpj) {
	        this.webClientcnpj = webClientcnpj;
	    }


	public CnpjDTOAPI buscarcnpj(String cnpjfront) {

		Mono<CnpjDTOAPI> cnpj = this.webClientcnpj.method(HttpMethod.GET).uri("/{codigo}", cnpjfront).retrieve()
